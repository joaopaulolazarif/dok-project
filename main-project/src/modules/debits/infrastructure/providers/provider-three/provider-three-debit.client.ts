...
import { ProviderThreeMapper } from './provider-three-debit.mapper';

export class ProviderThreeDebitClient implements IDebitExternalProvider {
  private readonly logger = new Logger(ProviderThreeDebitClient.name);
  ...
  getDebits(plate: string): Promise<VehicleDebits> {
    return this.circuitBreaker.fire('provider-three', () => this.fetchDebits(plate));
  }

  private async fetchDebits(plate: string): Promise<VehicleDebits> {
    const url = `${this.baseUrl}/debits`;
    const start = Date.now();
    this.logger.debug('outbound HTTP call', { provider: 'provider-three', method: 'POST', url, plate });
    ...
    try {
      ...
      return ProviderThreeMapper.toDomain(response.data);
    } catch (err) {
