...
import { DEBIT_PROVIDER_ONE, DEBIT_PROVIDER_THREE, DEBIT_PROVIDER_TWO } from './domain/providers/debit-provider.interface';
import {
  INTEREST_CALCULATOR,
...
import { ProviderThreeDebitClient } from './infrastructure/providers/provider-three/provider-three-debit.client';
...
@Module({
  ...
  providers: [
    ...
    {
      provide: DEBIT_PROVIDER_THREE,
      useFactory: (http: HttpService, cb: CircuitBreakerService) =>
        new ProviderThreeDebitClient(
          http,
          cb,
          process.env.PROVIDER_THREE_URL ?? 'http://localhost:3003',
        ),
      inject: [HttpService, CircuitBreakerService],
    },
    {
